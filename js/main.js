...
const init = () => {
  ...
  const clockCenter = draw.circle(5)
    .center(CENTER_X, CENTER_Y)
    .fill('rgba(34,34,34,0.48)')
    .css({"filter": "drop-shadow(3px 3px 3px #3C2003FF)"});


  const minuteHand = draw.line(CENTER_X, CENTER_Y, CENTER_X, CLOCK_MARGIN + NUMBER_MARGIN)
    .stroke({
      width: 2,
      color: "#000000"
    })
  ...
  const hourHand = draw.line(CENTER_X, CENTER_Y, CENTER_X, CLOCK_MARGIN + NUMBER_MARGIN * 2)
    .stroke({
      width: 4,
      color: "#000000"
    })
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    draw.line(CENTER_X, CENTER_Y, CENTER_Y, CENTER_Y - tick_length)
      .stroke({width: 0, color: "black"})
      .rotate(angleInDeg)
      .dy(-1 * (CLOCK_RADIUS - (tick_length / 2) * Math.cos(angleInRad)))
      .dx((tick_length / 2) * (Math.sin(angleInRad)));
